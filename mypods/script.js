...
function openCart() {
  ...
  // Guardar elemento que disparou a abertura e estado de scroll
  ...
  // Bloquear rolagem do body enquanto o modal estiver aberto
  ...
  if (!modal.getAttribute('aria-labelledby')) {
    ...
    if (title) {
      if (!title.id) title.id = 'cartTitle';
  ...
  const focusArray = Array.from(focusables).filter(el => !el.hasAttribute('disabled'));
  ...
  const onKeyDown = (e) => {
    if (e.key === 'Escape') {
      e.preventDefault();
      closeCart();
    } else if (e.key === 'Tab' && focusArray.length > 0) {
  ...
  modal._cartHandlers = { onKeyDown, prevOverflow, returnFocus: returnFocusEl };
...
function closeCart() {
  ...
  if (modal) {
    ...
    if (modal._cartHandlers?.prevOverflow !== undefined) {
      document.body.style.overflow = modal._cartHandlers.prevOverflow;
    }

    // Remover listeners
    if (modal._cartHandlers?.onKeyDown) {
      modal.removeEventListener('keydown', modal._cartHandlers.onKeyDown);
    }

    // Restaurar foco ao elemento que abriu o modal, se ainda existir
    if (modal._cartHandlers?.returnFocus && document.contains(modal._cartHandlers.returnFocus)) {
      try { modal._cartHandlers.returnFocus.focus(); } catch (_) {}
    }

    delete modal._cartHandlers;
  }
}
